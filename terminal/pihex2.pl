...
while ($start < length $pi) {
    ...
    $width = ((GetTerminalSize)[0] - 1) & ~1;
    print "\e[0m", substr($pi, $start, $width), "\n";
    for my $bit (0..7) {
        my $color = $colors[$bit];
        my $prev = $prev[$bit];
        my $pos = 0;
        while ($pos < $width) {
            ...
            if ($slice !~ /[0-9A-F]/) {
                ...
                $color++ if $slice != $prev;
                $prev = $slice;
                printf "\e[1;%dm", 30 + ($color & 7);
                print ($slice ? $fill : $empty);
            }
            $pos += 2;
        }

        # Nice code duplication
        my $slice = substr $pi, $start, 2;
        if ($slice =~ /[0-9A-F]/) {
            my $curr = ((hex $slice) >> $bit) & 1;
            $colors[$bit]++ if $curr != $prev[$bit];
            $prev[$bit] = $curr;
        }
        print "\n";
    }
